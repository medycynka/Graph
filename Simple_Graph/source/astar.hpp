...
namespace ads::algo::ga {
    ...
    std::pair<double, std::vector<std::size_t>> astar(ads::ds::graph::Graph<V, E>& graph, std::size_t start_idx, std::size_t end_idx, std::function<double(const ads::ds::graph::Graph<V, E>&, std::size_t actual_vertex_id, std::size_t end_vertex_id)> heuristics, std::function<double(const E&)> getEdgeLength) {
