...
inline std::pair<double, std::vector<size_t>> Graph<V, E>::AStar(size_t start, size_t end, std::function<double(const E&)> visitator_f, std::function<double(const Graph<V, E>&, const size_t&, const size_t&)> heuristic) const{
    ...
    while(!nodes.empty()){
        ...
        for(auto i = 0; i < size_; i++){
            auto alt = distances[smallest] + visitator_f(neigh_matrix.at(smallest).at(i).value_or(INF)) + heuristic(*this, smallest, i);
            //auto alt = visitator_f(std::sqrt((smallest - end)*(smallest - end) + (i - end)*(i - end)) - heuristic(*this, smallest, i));
