...
public:
    ...
    /*std::pair<EdgesIterator, bool>*/ bool insertEdge (size_t vertex1_id, size_t vertex2_id, const E &label = E(), bool replace = true);
    bool removeVertex(size_t vertex_id);
    bool removeEdge(size_t vertex1_id, size_t vertex2_id);
    bool edgeExist(size_t vertex1_id, size_t vertex2_id) const { return neigh_matrix[vertex1_id][vertex2_id]; };
    size_t nrOfVertices()                                const { return vertices.size(); };
    size_t nrOfEdges()                                   const { return no_of_edges; };
    ...
    VerticesIterator beginVertices() { return VerticesIterator(*this, 0); };
    VerticesIterator endVertices()   { return VerticesIterator(*this, vertices.size()); };
    EdgesIterator beginEdges()       { return EdgesIterator(*this, 0, 0); };
    EdgesIterator endEdges()         { return EdgesIterator(*this, vertices.size(), vertices.size()); };
...
typename Graph<V, E>::EdgesIterator &Graph<V, E>::EdgesIterator::operator++(){
    if(curr_col < reference.vertices.size() - 1){
        curr_col++;
        pom++;

        while(!reference.neigh_matrix.at(curr_row).at(curr_col) && curr_col < reference.vertices.size()) {
            curr_col++;
            pom++;
        }
    }
    else{
        if(pom < (reference.nrOfVertices() * reference.nrOfVertices())) curr_col = 0;
        curr_row++;
...
template <typename V, typename E>
bool Graph<V, E>::insertEdge(size_t vertex1_id, size_t vertex2_id, const E &label, bool replace){
...
template <typename V, typename E>
bool Graph<V, E>::removeVertex(size_t vertex_id){
...
template <typename V, typename E>
bool Graph<V, E>::removeEdge(size_t vertex1_id, size_t vertex2_id){
    auto& currentEdge = neigh_matrix[vertex1_id][vertex2_id];
